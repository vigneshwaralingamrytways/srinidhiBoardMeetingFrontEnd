...
import { FaCheck, FaPencilAlt, FaTasks, FaFilePdf, FaFileImage, FaFile, FaFileWord, FaFileExcel, FaTimes, FaDownload, FaEye, FaPlus, FaEnvelope, FaPaperPlane, FaExclamationCircle, FaRegClock, FaTrash } from "react-icons/fa";
...
function DocViewerModal({ doc, onClose }) {
  ...
  return (
    <div
      style={{ position:"fixed", inset:0, zIndex:10000, background:"rgba(0,0,0,0.72)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}
      onClick={onClose}
    >
      <div
        style={{ width:"min(880px,96vw)", maxHeight:"92vh", borderRadius:20, overflow:"hidden", display:"flex", flexDirection:"column", background:"#090c20", border:"1px solid rgba(255,255,255,0.08)" }}
      ...
      >
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 22px", borderBottom:"1px solid rgba(255,255,255,0.07)", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <DocIcon name={label} size={18} />
            <div>
              <div style={{ color:"#f4f0ff", fontWeight:800, fontSize:14 }}>{label}</div>
              <div style={{ color:"#596197", fontSize:11 }}>Agenda Document</div>
            </div>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            {url && (
              <a href={url} download={label}
                style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"7px 14px", borderRadius:9, border:"1px solid rgba(212,168,83,0.35)", color:"#D4A853", background:"rgba(212,168,83,0.08)", fontSize:12, fontWeight:700, textDecoration:"none" }}>
                <FaDownload /> Download
              </a>
            )}
            <button onClick={onClose} style={{ width:32, height:32, borderRadius:8, background:"transparent", border:"1px solid rgba(255,255,255,0.10)", color:"#596197", cursor:"pointer", display:"grid", placeItems:"center", fontSize:14 }}>
              <FaTimes />
            </button>
          </div>
        </div>
        ...
        {/* Preview body */}
        <div style={{ flex:1, overflow:"auto", background:"#07091a" }}>
          {!url ? (
            <div style={{ display:"grid", placeItems:"center", height:300, color:"#3d4570", fontSize:13 }}>No preview available</div>
          ) : isPdf ? (
            <iframe src={url} title={label} style={{ width:"100%", height:520, border:"none" }} />
          ) : isImage ? (
            <img src={url} alt={label} style={{ maxWidth:"100%", display:"block", margin:"0 auto", padding:24 }} />
          ) : (
            <div style={{ display:"grid", placeItems:"center", height:300, textAlign:"center", padding:32 }}>
              <div>
                <FaFile style={{ fontSize:48, color:"#3d4570", marginBottom:16 }} />
                <div style={{ color:"#f4f0ff", fontWeight:700, marginBottom:8 }}>{label}</div>
                <div style={{ color:"#596197", fontSize:13, marginBottom:20 }}>This file type cannot be previewed. Download to open it.</div>
                <a href={url} download={label}
                  style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"10px 22px", borderRadius:10, border:"1px solid rgba(212,168,83,0.35)", color:"#D4A853", background:"rgba(212,168,83,0.10)", fontSize:13, fontWeight:700, textDecoration:"none" }}>
...
export default function AgendaDiscussion({
  ...
  const handleNavigateToRes = id => { setResAgendaId(id); goToStep(5); };
  const handleNavigateToDec = id => { setDecAgendaId(id); goToStep(6); };
