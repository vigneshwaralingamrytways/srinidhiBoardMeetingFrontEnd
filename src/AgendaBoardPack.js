...
import {
  ...
  FaSitemap, FaGlobe, FaFile, FaFilePdf, FaFileImage, FaFileWord, FaFileExcel, FaFilePowerpoint,
  FaChevronRight, FaDownload,
} from "react-icons/fa";
import { getAgendaTitleById } from "./ConductMeetingData";
...
function FileTypeBadge({ type }) {
  const map = {
    ...
    PPT: { color: "#e8943a", bg: "rgba(232,148,58,0.12)", Icon: FaFilePowerpoint },
  };
...
/* -------------------------------------------------------------------
   Sample board pack data  covers both buckets:
   - "common" docs (agendaId: null) shared across every agenda item
   - "agenda related" docs tied to whichever agenda item this panel is
     currently scoped to (the agendaId passed in as a prop)

   The PPT sample carries a `slides` array so it opens in the same
   DocZoomViewer "projector" mode Resolutions already use, and a
   `fullWidth` flag so the viewer can render it edge-to-edge.
------------------------------------------------------------------- */
function buildSampleBoardPack(agendaId) {
  return [
    {
      id: "bp-common-1",
      title: "Board Charter & Governance Policy.pdf",
      type: "PDF",
      pages: 18,
      size: "1.4 MB",
      agendaId: null,
    ...
    {
      id: "bp-common-2",
      title: "Audit Committee Minutes - Q3.docx",
      type: "DOC",
      pages: 6,
      size: "320 KB",
      agendaId: null,
    ...
    {
      id: "bp-agenda-1",
      title: "Q4 Financial Overview.pptx",
      type: "PPT",
      pages: 12,
      size: "4.8 MB",
      agendaId,
      ...
      fullWidth: true,
      slides: [
        { title: "Q4 Financial Overview", body: "<p>Summary of revenue, costs, and margin trends for the quarter under discussion.</p>" },
        { title: "Revenue Breakdown", body: "<p>Performance by region and product line compared against forecast.</p>" },
        { title: "Outlook & Recommendations", body: "<p>Key risks, opportunities, and proposed actions for the board to consider.</p>" },
      ],
    },
    {
      id: "bp-agenda-2",
      title: "Supporting Data Pack.pdf",
      type: "PDF",
      pages: 9,
      size: "2.1 MB",
      agendaId,
...
export default function BoardPackPanel({ agendaId, agendaName }) {
  const [docs] = useState(() => buildSampleBoardPack(agendaId));
    ...
    <div style={S.wrap}>
      ...
      {viewerDoc && (
        <DocZoomViewer
          ...
          fullWidth={viewerDoc.type === "PPT" || !!viewerDoc.fullWidth}
